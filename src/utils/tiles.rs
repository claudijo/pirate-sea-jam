...
pub fn smoothen_edges(mut positions: Vec<[f32; 3]>, subdivisions_count: u32) -> Vec<[f32; 3]> {
    ...
    for i in 0..positions.len() {
        ...
        } else if row == subdivisions_count {
            // Last row
            let order = i % VERTICES_PER_QUAD;
        ...
        if col == 0 {
            ...
            if row % 2 == 0 {
                ...
                if order == 2 {
                    let midpoint =
                        get_midpoint(positions[i + 1], positions[i + size * VERTICES_PER_QUAD]);
                    positions[i] = midpoint;
                }
        ...
        } else if col == subdivisions_count {
            // Last col
            let order = i % VERTICES_PER_QUAD;
            ...
            if row % 2 == 0 {
                ...
                if order == 5 {
                    let midpoint =
                        get_midpoint(positions[i - size * VERTICES_PER_QUAD], positions[i - 5]);
                    positions[i] = midpoint;
                }
            }
        }
