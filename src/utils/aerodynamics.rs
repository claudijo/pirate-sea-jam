use crate::utils::linear_algebra::{angle_between_perpendicular, perpendicular_to_projection_direction};
use bevy::math::{Vec3};
use std::ops::Neg;
...
pub fn scaled_lift_drag(relative_velocity: Vec3, surface_normal: Vec3) -> (Vec3, Vec3) {
    ...
    let lift =  perpendicular_to_projection_direction(relative_velocity, normal).normalize() * simple_lift_coefficient(angle);
