...
pub fn buoyancy(
    ...
    let elapsed_time = time.elapsed().as_secs_f32();
    for (transform, pontoon_size, velocity, pontoon_force_scale, mut external_force, mut damping) in &mut pontoons
    {
        ...
        let linear_damping = if is_submerged {
           liquid::damping(velocity.linvel.y, pontoon_size.side.powi(2), CUBE_DRAG_COEFFICIENT)
        } else {
            0.
        };
 
        external_force.force = buoyant_force * pontoon_force_scale.buoyant_force_scale;
        damping.linear_damping = linear_damping * pontoon_force_scale.linear_damping_scale;
    }
}
